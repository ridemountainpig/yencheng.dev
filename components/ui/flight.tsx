...
function FlightAirport({
    ...
    const displayName =
        nameProp ?? (airportInfo ? airportInfo.code : undefined);
...
function FlightRoute({
    ...
    const resolvedDash = useMemo(
        () => resolveDashArray(lineStyle),
        [lineStyle],
    );
    ...
    useEffect(() => {
        ...
        const handleMouseEnter = (
            e: MapLibreGL.MapLayerMouseEvent & object,
        ) => {
        ...
        <>
            {routes.map((route, index) => (
                <FlightRoute
                    ...
                    onClick={
                        route.onClick ??
                        (onClick ? () => onClick(index, route) : undefined)
                    }
                    onMouseEnter={
                        route.onMouseEnter ??
                        (onMouseEnter
                            ? () => onMouseEnter(index, route)
                            : undefined)
                    }
                    onMouseLeave={
                        route.onMouseLeave ??
                        (onMouseLeave
                            ? () => onMouseLeave(index, route)
                            : undefined)
                    }
                    showAirports={false}
                />
...
function FlightAnimationMarker({
    ...
    const updateMarker = useCallback(
        (t: number) => {
            if (!arcCoords || arcCoords.length < 2 || !markerRef.current)
                return;
...
function FlightMultiLegAnimationMarker({
    ...
    const legData = useMemo(() => {
        ...
        for (const leg of legs) {
            ...
            breakpoints.push(
                totalDistance > 0 ? cumulative / totalDistance : 0,
            );
        }
    ...
    const updateMarker = useCallback(
        (t: number) => {
            ...
            const { pos, bearing: deg } = computePositionAndBearing(
                segmentT,
                data,
            );
            markerRef.current.setLngLat(normalizeLngLat(pos));
